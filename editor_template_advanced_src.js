	...
	tinymce.create('tinymce.themes.AdvancedTheme', {
		...
		init : function(ed, url) {
			...
			if (ed.settings.content_css !== false) {
				ed.contentCSS.push(tinymce.baseURL + "/skin_" + ed.settings.skin + "_content.css");
				}
			// Init editor
			...
			DOM.loadCSS(s.editor_css ? ed.documentBaseURI.toAbsolute(s.editor_css) : tinymce.baseURL+"/skin_"+ed.settings.skin+"_ui.css");
												
			if (s.skin_variant)
				DOM.loadCSS(tinymce.baseURL + "/skin_"+ed.settings.skin + "_ui_" + s.skin_variant + ".css");
		},	
...
*/
		...
		_simpleLayout : function(s, tb, o, p) {
			...
			//modified to add tabs row
			if (!s.theme_advanced_toolbar_container) {
				n = DOM.add(tb, 'tr', {'class' : 'mceTabsRow'}); 
				n = DOM.add(n, 'td', {'id' : 'mceTabs'});
				n = DOM.add(tb, 'tr');
				n = ic = DOM.add(n, 'td', {'class' : 'mceIframeContainer'});
			}
