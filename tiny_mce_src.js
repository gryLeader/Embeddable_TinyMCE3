...
(function(win) {
	...
	var tinymce = {
		...
		_init : function() {
			...
			t.isltIE9 = document.all && !document.addEventListener;
		...
		setAttrib : function(e, n, v) {
			...
			return this.run(e, function(e) {
				...
				if (is(v) && v !== null && v.length !== 0) {
					if (tinyMCE.isltIE9) {
					//modified: IE8-7 displayed error when setting attr aria-hidden						
						if (n !== 'aria-hidden')
...
(function(tinymce) {
	...
	tinymce.create('tinymce.AddOnManager', {
		...
		requireLangPack : function(n,prefx) {
			var s = tinymce.settings;
			
			if (s && s.language && s.language_load !== false && n == 'advanced') {
				tinymce.ScriptLoader.add(s.language + '_advanced.js') 
			}
			else if (s && s.language && s.language_load !== false && n == 'simple') {
				tinymce.ScriptLoader.add(s.language + '_simple.js') 
			}			
			else if (s && s.language && prefx && s.language_load !== false) {
				tinymce.ScriptLoader.add(this.urls[n] + '/' + s.language + '_' + prefx + '.js')
			}
			else if (s && s.language && s.language_load !== false)
				tinymce.ScriptLoader.add(this.urls[n] + '/' + s.language + '.js');			
		},
...
(function(tinymce) {
	...
	tinymce.create('tinymce.Editor', {
		...
		render : function(nst) {
			...
			function loadScripts() {
				if (s.language && s.language_load !== false) {
					sl.add(tinymce.baseURL + '/' + s.language + '.js');
				}

				if (s.theme && typeof s.theme != "function" && s.theme.charAt(0) != '-' && !ThemeManager.urls[s.theme]) {
					ThemeManager.load(s.theme, 'editor_template_' + s.theme + tinymce.suffix + '.js');
				}
					...
					if (p &&!PluginManager.urls[p]) {
						if (p.charAt(0) == '-') {
							...
							each(dependencies, function(dep) {
								var defaultSettings = {prefix:'plugin_', resource: dep, suffix:'_editor' + tinymce.suffix + '.js'};
							...
							PluginManager.load(p, {prefix:'plugin_', resource: p, suffix:'_editor' + tinymce.suffix + '.js'});
						}
					}
				});
		...
		init : function() {
			...
			if (s.popup_css !== false) {
				if (s.popup_css) {
					...
					s.popup_css = t.baseURI.toAbsolute("skin_"+s.skin+"_dialog.css");
				}
			}
		...
		getBody : function() {
			var temporaryel = this.bodyElement || this.getDoc().body;
			
			if (typeof temporaryel == 'undefined') {				
				return this.bodyElement || this.getDoc().body;
			}			
			else {					
				return temporaryel;
				}				
			//mod exe modifique esta función... 			
		},
	...
	tinymce.Formatter = function(ed) {
		...
		function defaultFormats() {
			register({
				...
				removeformat : [
					{selector : 'b,strong,em,i,font,u,strike,sup,sub', remove : 'all', split : true, expand : false, block_expand : true, deep : true},
